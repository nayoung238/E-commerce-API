...
public class KafkaProducerService {
    ...
    private final KafkaTemplate<String, OrderUpdatedEvent> orderUpdatedEventKafkaTemplate;
    ...
    public void send(String topic, @Payload(required = false) OrderUpdatedEvent value) {
        orderUpdatedEventKafkaTemplate.send(topic, value)
            .whenComplete((orderUpdatedEventSendResult, throwable) -> {
                if(throwable == null) {
                    RecordMetadata metadata = orderUpdatedEventSendResult.getRecordMetadata();
                    internalEventService.updatePublicationStatus(value.orderEventId(), InternalEventStatus.send_success);
                    log.info("Kafka event published successfully -> topic: {}, partition: {}, offset: {}, orderEventKey: {}",
                        metadata.topic(),
                        metadata.partition(),
                        metadata.offset(),
                        orderUpdatedEventSendResult.getProducerRecord().key());
                } else {
                    internalEventService.updatePublicationStatus(value.orderEventId(), InternalEventStatus.send_fail);
                    log.error("Failed to publish Kafka event: " + throwable.getMessage());
                }
            });
    }
